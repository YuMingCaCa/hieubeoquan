...
export async function hienThiBaoCaoTongQuan() {
    ...
    const [totalRevenue, totalCost, salesDetails, costDetails] = await Promise.all([
        getTotalAmountInRange(['restaurants', restaurantId, 'sales_history'], 'ngay_thanh_toan', 'tong_tien', startDate, endDate),
        getTotalAmountInRange(['restaurants', restaurantId, 'purchase_history'], 'ngay_nhap', 'thanh_tien', startDate, endDate),
        getDocs(query(collection(db, 'restaurants', restaurantId, 'sales_history'), where('ngay_thanh_toan', '>=', startDate), where('ngay_thanh_toan', '<', endDate), orderBy('ngay_thanh_toan', 'desc'))),
        getDocs(query(collection(db, 'restaurants', restaurantId, 'purchase_history'), where('ngay_nhap', '>=', startDate), where('ngay_nhap', '<', endDate), orderBy('ngay_nhap', 'desc')))
    ]);
...
async function suaChiPhi(e) {
    ...
    const chiPhiRef = doc(db, "restaurants", restaurantId, "purchase_history", id);
