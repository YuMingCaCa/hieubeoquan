...
async function xoaMonKhoiMenu(e) {
    ...
    if (confirm(`Bạn có chắc chắn muốn xóa món "${tenMon}" khỏi menu không?`)) {
        try {
            await deleteDoc(doc(db, 'restaurants', restaurantId, 'menu', monId));
...
async function suaGiaMon(e) {
    ...
    try {
        const monRef = doc(db, 'restaurants', restaurantId, 'menu', monId);
...
export function initMenu() {
    ...
    const q = query(collection(db, 'restaurants', restaurantId, 'menu'), orderBy('ten_mon'));
